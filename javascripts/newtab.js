...
(function() {
  ...
  ChromePicturesNewTab = (function() {
    ...
    ChromePicturesNewTab.prototype.poolThreshold = 50;

    ChromePicturesNewTab.prototype.filterByLicenses = true;

    ChromePicturesNewTab.prototype.filteredLicenses = [1, 2, 4, 5, 7];
    ...
    ChromePicturesNewTab.prototype.fetchPhoto = function() {
      return new RSVP.Promise((function(_this) {
        return function(resolve, reject) {
          return _this.fetchPhotos().then(function(photos) {
            var index, ownerName, ownerWebUrl, photo, title, webUrl;
    ...
    ChromePicturesNewTab.prototype.fetchPhotos = function(_date, _photos) {
      if (_date == null) {
        _date = null;
      }
      if (_photos == null) {
      ...
      if (this._cachedPhotos) {
        return RSVP.resolve(this._cachedPhotos);
      } else {
        if (!_date) {
          _date = new Date();
        }
        console.log("Fetching from " + (this.formatDate(_date)));
        return this.flickrApiRequest("flickr.interestingness.getList", {
          ...
          return function(resp) {
            var query;
            _photos = _photos.concat(_this.filterByLicenses ? (query = _.map(_this.filteredLicenses, function(license) {
              return "photo[license=" + license + "]";
            }), $(resp).find(query.join(",")).toArray()) : $(resp).find("photo").toArray());
            if (_photos.length < _this.poolThreshold) {
              return _this.fetchPhotos(new Date(_date.getTime() - 86400000), _photos);
            } else {
              _this._cachedPhotos = _photos;
              return _this._cachedPhotos;
            }
          };
        })(this));
      }
    ...
    ChromePicturesNewTab.prototype.formatDate = function(date) {
      var dom, month, string;
      string = "" + (date.getFullYear());
      month = date.getMonth() + 1;
      dom = date.getDate();
      string = month < 10 ? "" + string + "-0" + month : "" + string + "-" + month;
      string = dom < 10 ? "" + string + "-0" + dom : "" + string + "-" + dom;
      return string;
    };
